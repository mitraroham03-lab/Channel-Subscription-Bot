...
- You purchase the Code at your own risk and with full awareness of the high risks involved in algorithmic trading.
...
def show_disclaimer(call):
    ...
    markup = InlineKeyboardMarkup()
    # Pass the channel and duration to the next step via callback_data
...
def create_payment(call):
    """Generates the actual payment link after user agrees to terms."""
    _, ch_id, mins = call.data.split("_")
...
def process_forward(message):
    ...
    msg = bot.reply_to(message, f"Channel: {ch_name}\nEnter plans in format `Minutes:Price` (e.g., `1440:99, 10080:500`)")
